        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url,  cookies=config.MPSTATS_COOKIES, proxy=config.PROXY, timeout=10) as response:
                ...
                if response.status != 200:
                    ...
                    sql = f"UPDATE subject SET subject_price_segmentation_raw=(%s) WHERE subject_id = (%s)"
...
async def main():
    with psycopg.connect(host=config.DB_HOST,
        ...
        with conn.cursor() as cur:
            ...
            df = pd.read_sql_query('select subject_id from subject where subject_price_segmentation_raw is NULL', con=conn)
...
while True:
    try:
        asyncio.run(main())
        print(datetime.datetime.now(), 'сбор завершен - сегментация цены')
