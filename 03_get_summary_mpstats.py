...
async def get_subject_summary(id, cur):
    try:
        url = f'https://mpstats.io/api/wb/get/subjects/summary/{id}?fbs=0'
        async with aiohttp.ClientSession() as session:
            async with session.get(url,  cookies=config.MPSTATS_COOKIES, proxy=config.PROXY, timeout=10) as response:
                ...
                if response.status != 200:
                    ...
                    sql = f"UPDATE subject SET subject_summary_raw=(%s) WHERE subject_id = (%s)"
...
async def main():
    with psycopg.connect(host=config.DB_HOST,
        ...
        with conn.cursor() as cur:
            ...
            df = pd.read_sql_query('select subject_id from subject where subject_summary_raw is NULL', con=conn)
